...
This project uses ESLint v9 with the flat config format (`eslint.config.mjs`).
Key points:

- **Flat Config Format**: All ESLint configurations use the flat config array
  format (no `.eslintrc.json`)
- **TypeScript ESLint**: Uses the `typescript-eslint` meta-package (v8+) instead
  of separate parser/plugin
- **Root Configuration**: The root `eslint.config.mjs` provides base rules,
  plugin configurations, and module boundaries
- **Project Configurations**: Each package extends the root config using
  `import baseConfig from '../../eslint.config.mjs'`

### Adding ESLint Configuration to Generators

When creating generators that need to configure ESLint:

1. **Use `mergeEslintConfig` from `@ensono-stacks/core`**: This utility handles:
    - Merging configs into existing `eslint.config.mjs` files
    - Adding required import statements
    - Reusing existing import variable names (e.g., `nextEslintPluginNext`)
    - Detecting and avoiding duplicate configurations
...
3. **Variable Name Reuse**: The merge utility automatically:
    - Detects existing imports (e.g., `@next/eslint-plugin-next`)
    - Extracts their variable names (e.g., `nextEslintPluginNext`)
    - Replaces your snippet's variable names to match existing ones
    - This prevents "variable is not defined" errors

4. **Duplicate Detection**: Configs are considered duplicates if they share:
    - The same `name` property, OR
    - The same `files` pattern, OR
    - Both are unnamed configs with identical plugin keys

### Linting Commands

- Lint all packages: `pnpm exec nx run-many -t lint --all`
- Lint affected packages: `pnpm exec nx affected -t lint`
- Lint with auto-fix: `pnpm exec nx run-many -t lint --fix`
- Lint specific package: `pnpm exec nx lint <package-name>`
...
- @ensono-stacks/_plugin_:[additional] (Any additional configuration or code
  ...
  functionality or behaviour to the users project should be split into their own
  generator. Alternatively, if the additional behaviour is only small, you could
  also consider adding this as a optional part of the init schema).
...
- _init_ generator: Use Git to review the differences between the
  ...
  files your generator needs to manipulate or create, then write all of the unit
  tests to check that the desired end state is met.
...
- **Option 1**: [pnpm linking](https://pnpm.io/cli/link)
    ...
    - From the workspace where you want to test your plugin you should now see
      the symlink present within your node_modules. As the link has taken place,
      for further changes simply running a new build as in step 1 should be
      enough, without having to go through steps 2 & 3 again. If not, repeat the
      steps.
...
- **prerelease**: Creates an alpha release of all packages to the `@dev` tag
    - When a package is merged to `main` the
      [prerelease.yml](/.github/workflows/prerelease.yml) workflow is run, this
      calls `pnpm release --specifier=prerelease`. A Prerelease Github Realease
      is created.
- **release**: Creates a production release of all the packages to the `@latest`
  tag
